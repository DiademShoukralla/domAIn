...
domAIn v1 is single-user / single-project, but the data model must not paint us into a corner. Knowledge sources, connections, and API keys need scoping (whose data is this? which project does it belong to?), and every read path must enforce access consistently. We evaluated a dedicated Permissions entity and simpler alternatives.
...
class KnowledgeSource(Base):
    id: UUID
    name: str
    source_type: SourceType       # "github_repo" | "linear"
...
`Connection` always requires `user_id` (OAuth credentials are inherently user-owned) and is not nullable.
...
- Connection list/get/delete endpoints
- Retrieval pipeline (filters chunks by accessible sources)
- Session/API key validation (key's `user_id` / `project_id` become the actor context)
